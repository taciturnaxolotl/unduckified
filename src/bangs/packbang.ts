...
// The edge redirect answers the first search of a profile, before any service
// worker exists, so it needs this same catalog. It rides along in the Function
// bundle rather than being fetched: /bangs.bin ships pre-compressed with
// `Content-Encoding: br` (see public/_headers) and the Workers runtime has no
// brotli decoder, so an asset read would hand back bytes it cannot parse.
// Base64 because a module is text. Decoded once per isolate.
writeFileSync(
	"src/bangs/catalog-embed.ts",
	`// Generated by packbang.ts. Do not edit.\n` +
		`// bangs.bin verbatim, for the edge redirect in functions/index.ts.\n` +
		`export const CATALOG_B64 = ${JSON.stringify(Buffer.from(flat).toString("base64"))};\n`,
);
